...
> **Última atualização:** 2026-06-03 (S27 Modo Objetivo deployed em prod + S28 Onboarding hand-holding commitado)
...
> - **S28 Onboarding hand-holding** (iOS, commit `7c34861`, 2026-06-03): substitui o tutorial passivo de 3 telas por `OnboardingFlow` de 6 telas onde o médico EXECUTA o primeiro laudo real em ~60s (mic via SpeechService → SSE generate → laudo salvo no histórico). Categoria fixa ABDOMEN_TOTAL, animações iOS 17+, microcopy sem jargão. dex1 10min + ajustes c1. `OnboardingView` antigo → `_LEGACY` @deprecated. Aditivo (não afeta Build 77). **Ainda não em build instalável** — entra no próximo archive.
> - **S27 Modo Objetivo** (backend monorepo, deploy prod `d7f196a`, 2026-06-03): novo estilo de escrita OBJETIVO enumerativo (TÉCNICA/ANÁLISE/OPINIÃO, pegada nReport/Laudário) — fork dos contracts MODELO_OBJETIVO em 6 categorias + suíte golden 19/20 (gate=19). Fix clínico S27.1: regra de IG-em-semanas-sem-data no OBSTETRICA, que corrige conflito da regra de DUM (derrubava idade gestacional / emitia placeholders `__/__/____`) no objetivo E no clássico em prod — verificado live. App consome via `writing_style_id`.
...
| 27 (backend) | ✅ | **Modo Objetivo enumerativo** — estilo OBJETIVO (TÉCNICA/ANÁLISE/OPINIÃO) via fork dos contracts MODELO_OBJETIVO em 6 categorias + overlay + post-processor determinístico + suíte golden 19/20 (gate=19, resíduo estocástico). **S27.1 fix clínico**: IG-em-semanas-sem-data no OBSTETRICA — corrige conflito da regra de DUM que derrubava a idade gestacional / emitia placeholders, no objetivo E no clássico em prod. Deploy prod `d7f196a`, verificado live. Backend monorepo; app consome via `writing_style_id`. |
| 28 | ✅ | **Onboarding hand-holding** — `Features/Onboarding/OnboardingFlow` (6 steps): médico executa o 1º laudo real em ~60s (SpeechService → `ReportService.generateStream` SSE → histórico). Categoria fixa ABDOMEN_TOTAL, error handling completo (mic negado/falha/clarify), animações iOS 17+, microcopy sem "token"/"backend". `OnboardingView`→`_LEGACY` @deprecated, integrado em `AppShellView`. dex1 10min + microcopy/integração c1. Commit `7c34861`, aditivo (não bloqueia Build 77). Entra no próximo archive. |
| DET-4 | ✅ | **Seletor de máscara nas Preferências** (sprint iOS do plano laudos determinísticos, 2026-06-12) — `Models/ReportPreference.swift` (novo) + `ProfileService.fetchReportPreferences()/updateReportPreference()` (GET/PATCH `/api/me/report-preferences`, null explícito limpa preferência) + `AppState.reportPreferences/availableVariants/refreshReportPreferences()` + seção "Modelos de laudo" no `SettingsView` (picker por categoria com >1 variante NO ESTILO ATUAL, dedup por `variant_key`, opção "Automático"). Reviews dex1+dex2 aplicados (filtro por estilo evita BUNDLE_VARIANT_EMPTY; guard signOut race; guard double-tap). E2E Simulator (dex1 via xcodebuildmcp): Enxuta→"LAUDO RESUMIDO", Automático→padrão. Evidência `/tmp/det4-e2e/`. |
